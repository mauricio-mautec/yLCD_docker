...
class weatherDB(object):
    ...
    def setNewCollectData (self, source, location):
        ...
        curs    = conn.cursor()
        stmt    = """INSERT INTO collectData VALUES (datetime(CURRENT_TIMESTAMP, 'localtime'), (?), (?))"""
        curs.execute (stmt, (source, location));
        conn.commit()

        stmt    = "select rowid from collectData ORDER BY ROWID DESC LIMIT 1"
        curs    = conn.cursor()
        curs.execute (stmt);
        collectData = curs.fetchone()
        conn.commit()
        conn.close()
        self.collectData = collectData[0]
    ...
    def getCollectData (self, location, source):
        ...
        conn.commit()
        curs  = conn.cursor()
        stmt  = "SELECT MESURE, VALUE FROM WEATHER WHERE COLLECTDATA = ? ORDER BY MESURE"
